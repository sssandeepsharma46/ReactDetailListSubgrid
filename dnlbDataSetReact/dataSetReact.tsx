...
export interface IdnlbDataSet {
  name?: string;
  dataset?: DataSet;
}

interface IdnlbDataSetState {
...
export class dnlbDataSet extends React.Component<
  IdnlbDataSet,
  IdnlbDataSetState
> {
  constructor(props: IdnlbDataSet) {
    super(props);
  ...
  componentDidUpdate(prevProps: IdnlbDataSet) {
  ...
  private getColumns(items: Record<string, unknown>[]): IColumn[] {
    ...
    return this.props.dataset.columns.map((column) => ({
      ...
      onRender: (item: Record<string, unknown>) => (
        <span>{item[column.name] ?? "-"}</span> // Ensure column values are displayed correctly
      )
    }));
  }
